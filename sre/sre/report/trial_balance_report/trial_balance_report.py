# Copyright (c) 2026, ashuar and contributors
...
def prepare_data(accounts, filters, parent_children_map, company_currency):
	...
	selected_levels = get_selected_levels(filters)

	for d in accounts:
	
		if selected_levels and d.indent not in selected_levels:
			continue
		...
		adjusted_indent = calculate_adjusted_indent(d.indent, selected_levels)
...
def get_selected_levels(filters):
	"""Parse and return selected levels from filter"""
	if not filters.get("account_levels"):
		return None
	
	selected_levels = []
	levels = filters.get("account_levels")
	...
	for level in levels:
		try:
			...
			selected_levels.append(int(level) - 1)
		except (ValueError, TypeError):
			pass
	
	return selected_levels if selected_levels else None


def calculate_adjusted_indent(current_indent, selected_levels):
	"""Calculate adjusted indent for display"""
	...
	try:
		position = sorted_levels.index(current_indent)
		return position
	except ValueError:
		return current_indent
